...
def root_cert_checker():
    if('tak' in os.listdir('/opt') and 'files' in os.listdir('/opt/tak/certs')):
        print('\n////////// updating root certification status ///////////')
        ...
        print('\n////////// resetting root certification status ///////////')
...
def make_root_ca():
    ...
    subprocess.run(['./makeRootCa.sh', '--ca-name', root_ca])
    print('\n////////// root certificate authority was created:', root_ca, ' //////////')
    # add feature to print where the cert was created & name of the cert

def make_server_cert():
    subprocess.run(['./makeCert.sh', 'server', 'takserver'])
    print('\n////////// takserver certificate was created //////////')
...
def make_admin_certs():
    ...
    os.chdir('/opt/tak/certs')
    subprocess.run(['./makeCert.sh', 'client', 'tak_admin_{}'.format(o_u)])
    subprocess.run(['mv', 
...
def make_user_certs():
    ...
    if('user_certs' not in check_user_certs_folder):
        ...
        for count in range(1,q + 1):
            ...
            os.chdir('/opt/tak/certs')
            subprocess.run(['./makeCert.sh', 'client', 'user_{}'.format(count)])
            print('\n////////// created certs for user {} //////////'.format(count))
            subprocess.run(['mv', 
