...
print('\n\n\n\033[1;32mTAK Server Installation Tool\033[0m')
print('\033[1;34m>>> version: 1.1.0\033[0m')
print('\033[1;34m>>> developed by: I Corps D2I Team\033[0m\n')
...
b_uninstall['command'] = lambda: [tool_progress('start','Uninstalling TAK server...'),
...
close_button['command'] = lambda: [print('\n\033[1;31mclosing TAK Server Installation Tool...\033[0m\n'),
                                   window.destroy()]
    ...
    def check_thread():
        if thread.is_alive():
            ...
            if(intf.status['installation_started'] == True):
                tool_progress('start', 'Installing TAK server...')
        else:
            refresh()
...
def refresh():
    ...
    if('tak' in os.listdir('/opt')):
        if('certs' in os.listdir('/opt/tak') and 'files' in os.listdir('/opt/tak/certs')):
            ...
            user_cert_generate_button.config(state='disabled')
            user_certs_quantity_input.config(state='disabled')
            user_cert_status.set('Root / Server / Admin certificates required.')
    else:
